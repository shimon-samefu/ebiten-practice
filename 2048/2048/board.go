...
func (slice TileSort) Len() int {
	return len(slice)
}

func (slice TileSort) Less(i, j int) bool {
	return (slice[i].current.y > slice[j].current.y) ||
		(slice[i].current.y == slice[j].current.y && slice[i].current.x > slice[j].current.x)
}

func (slice TileSort) Swap(i, j int) {
...
func (b *Board) Move(dir Dir) error {
	...
	if dir == DirSort {
		...
		for t := range b.tiles {
			tmpSlice = append(tmpSlice, t)
			valueSlice = append(valueSlice, t.current.value)
		}
		sort.Sort(tmpSlice)
		sort.Slice(valueSlice, func(i, j int) bool {
			return valueSlice[i] < valueSlice[j]
		})
		for i := range tmpSlice {
			tmpSlice[i].current.value = valueSlice[i]
		}
		return nil
	}
	if dir == DirReset {
		b.tiles = map[*Tile]struct{}{}
		for i := 0; i < 2; i++ {
			if err := addRandomTile(b.tiles, b.size); err != nil {
				return err
			}
