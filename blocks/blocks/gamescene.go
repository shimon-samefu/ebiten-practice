...
func (s *GameScene) Update(state *GameState) error {
	if !s.gameover {
		if inpututil.IsKeyJustPressed(ebiten.Key1) {
			s.nextPiece = Pieces[BlockType1]
		} else if inpututil.IsKeyJustPressed(ebiten.Key2) {
			s.nextPiece = Pieces[BlockType2]
		} else if inpututil.IsKeyJustPressed(ebiten.Key3) {
			s.nextPiece = Pieces[BlockType3]
		} else if inpututil.IsKeyJustPressed(ebiten.Key4) {
			s.nextPiece = Pieces[BlockType4]
		} else if inpututil.IsKeyJustPressed(ebiten.Key5) {
			s.nextPiece = Pieces[BlockType5]
		} else if inpututil.IsKeyJustPressed(ebiten.Key6) {
			s.nextPiece = Pieces[BlockType6]
		} else if inpututil.IsKeyJustPressed(ebiten.Key7) ||
			inpututil.IsKeyJustPressed(ebiten.Key8) ||
			inpututil.IsKeyJustPressed(ebiten.Key9) ||
			inpututil.IsKeyJustPressed(ebiten.Key0) {
			s.nextPiece = Pieces[BlockType7]
		}
		if inpututil.IsKeyJustPressed(ebiten.KeyEscape) {
			s.gamesuspend = !s.gamesuspend
		}
		if s.gamesuspend {
			return nil
		}
...
func (s *GameScene) Draw(r *ebiten.Image) {
	...
	if s.nextPiece != nil {
		drawTextWithShadowCenter(r, "[Debug] Esc: Suspend, Num: Change", 0, ScreenHeight - 10, 1, color.NRGBA{0x80, 0, 0, 0xff}, ScreenWidth)
	}
}
